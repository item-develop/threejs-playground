...
void main(){
    ...
    float n1 = snoise(vec3(pos * (0.1), time*1.2));
    float n2 = snoise(vec3(pos * (0.2) + 100.0, time * 1.7));
    float n3 = snoise(vec3(pos * (0.2) - 50.0, time * 1.5));
    ...
    vec3 green2 = vec3(0.0, 0.9, 0.2);   // 中間の緑
    vec3 green3 = vec3(0.0, 0.9, 0.1);   // 暗い緑
    ...
    float colorNoise = snoise(vec3(pos * (1.0), time *0.2)) * 0.5 + 0.5;
    // さらにノイズで色に変化を加える
    color = mix(color, black, (1.0 - colorNoise) * (1.8-len));
