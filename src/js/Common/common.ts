...
float fbm(vec3 p, int octaves) {
  ...
  for (int i = 0; i < octaves; i++) {
    ...
    p += shift;            // オクターブ間のオフセットでパターン重複を避ける
    frequency *= 2.2;      // lacunarity: 高めで細かい粒が出やすい
    amplitude *= 1.04;     // persistence: 高周波の寄与をやや抑え目
  }
  return value;
}
...
vec3 hsv2rgb(vec3 c) {
  ...
  float fbm(vec3 x, int octaves) {
    ...
    for (int i = 0; i < octaves; ++i) {
        v += a * snoise(x * frequency);
        x = x * 2.0 + shift;
        frequency *= 2.2;    // lacunarity やや高め → 細かい粒が増える
        a *= 0.45;
    }
    return v;
}
...
export const finalFrag = `
              ...
              void main() {
                ...
                vec3 lightenBack = clamp(backColor.rgb * 1. + vec3(0.2, 0.2, 0.2), 0.0, 1.0);

               finalColor = frontColor.rgb * backColor.rgb;

                 if(
                  length(finalColor) < 0.8
                  ){
                    
                  // float distance = 0.8 - length(finalColor);
                  // finalColor += distance * vec3(0.8, 0.8, 0.8);
                  }
                ...
                //gl_FragColor = vec4(lightenBack.rgb, 1.0);
