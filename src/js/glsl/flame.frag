...
// Optional: Keep the original function as a convenience method
// that rotates around the center (0.5, 0.5)
vec2 rotateUv(vec2 uv, float angle) {
  return rotateUv(uv, angle, vec2(0.5, 0.));
}
...
void main() {
  ...
  *
    pow(  smoothstep(0.,0.8, pow(mouseDistance,1.)*1. ),2.)
     *1.4*pow(uv.y,2.) * (0.+min(uMouseStrength,0.1))*17.,
     
     vec2((1.* (uv.x*2.-1.)) + 1./ 2. , 0.)
      );
...
uv.x = uv.x + noise3*0.007 * uv.y;
 ...
 vec4 color = texture2D(uTexture,    uv+noise1*0.003* uv.y+ noise2*0.013);
  ...
  vec3 cNoise1 = curlNoise(vec3(uv.x*0.2,uv.y*0.2,uTime*0.1) );
  vec2 uv4 = vec2(uv.x + cNoise1.x*0.013*vUv.y,uv.y + cNoise1.y*0.013*vUv.y );

  gl_FragColor = texture2D(uTexture, uv+
  pow(  smoothstep(0.,6., pow(2./mouseDistance,1.)*1. ),2.) * (4.783)
  *
  noise1*0.001* uv.y+ noise2*0.013);


  //gl_FragColor = texture2D(uTexture, uv4);

//gl_FragColor = vec4(vec3(mouseDistance), uMouseStrength*40.);
