  ...
  createBoxes() {
    ...
    for (let x = -5; x <= 5; x += 2) {
      for (let z = -5; z <= 5; z += 2) {
        ...
        const about = document.querySelector('.kura-about')
        const body = document.body
        const aboutTop = about!.getBoundingClientRect().top - body.getBoundingClientRect().top
        ...
        const boxBody = new CANNON.Body({
          mass: 100,
          shape: boxShape,
          position: new CANNON.Vec3(x * 1.1, posY, z * 0.4 + (aboutTop - window.innerHeight / 2) / this.getWindowRate())
        });
  ...
  updatePhysics(deltaTime: number) {
    ...
    for (let i = 0; i < this.bodies.length; i++) {
      ...
      if (boxIds.includes(this.meshes[i].id) && !this.touchIds.includes(this.meshes[i].id)) {
        ...
        this.bodies[i].position.x -= deltaTime * 5
        if (this.bodies[i].position.x < -15) {
          this.bodies[i].position.x = 15
        }
        this.initialBoxes[this.meshes[i].uuid].prevX = this.bodies[i].position.x
        this.bodies[i].position.z += Math.sin((this.time - this.bodies[i].position.x) * 0.2) * 0.02
      }
